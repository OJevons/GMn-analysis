...
void HodoCalib(){
  ...
  for(Int_t i=0; i<nBarsTDC; i++) {
    ...
    if(fabs(vsc[i] - 0.173) > 0.06){
      htd0_post->Fill(i+1, -0.74);
      hvsc_post->Fill(i+1, 0.173);
    }
    else{
  ...
  TGraph* g_vsc = new TGraph("/home/oliver/SBS/SBS-replay/DB/vscint.txt","%lg \t %lg","");
  ...
  TGraph* g_tdo = new TGraph("/home/oliver/SBS/SBS-replay/DB/tdiffoff.txt","%lg \t %lg","");
  ...
  if(PlotPMT){
    ...
    TFitResultPtr fit_noTW = hpmtL_tdiff->Fit("gaus","SQ0");
    TFitResultPtr fit_TW = hpmtL_tdiff_TW->Fit("gaus","SQ0");
  ...
  gSystem->Exec("pdfunite hodo*.pdf /scratch/oliver/HodoCalib.pdf");
